...
def main():
    ...
    avg_k_max = 7
    times = 30
    all_avg_k = np.arange(start=avg_k_min, stop=avg_k_max, step=0.5)
    threshold_s = {}
    ...
    df.to_csv(path_or_buf=os.path.join(args.results_path, 'ER_diff_average_k(1).csv'))
    ...
    with plt.style.context(['science', 'ieee']):
        figure = plt.figure()
        # plt.title(r'The results on ER graph with $N={}$ and $\bar k \in (2,7)$'.format(args.N))
        ax = plt.subplot()
        ax.plot(df.index.values, df.iloc[:, 0], color='red', marker='v', label=r'$\lambda_c^{QMF}$')
        ax.plot(df.index.values, df.iloc[:, 1], color='blue', marker='o', label=r'$\lambda_c^{HMF}$')
        ...
        plt.legend()
        plt.savefig(os.path.join(args.results_path, 'ER_diff_average_k(1).png'))

        plt.figure()
        ax = plt.subplot()
        plt.title(r'Relative error between $\lambda_c^\prime$ and $\lambda_c^{HMF}$')
        ax.plot(all_index, RE_HMF, label=r'RE($\lambda_c^\prime$, $\lambda_c^{HMF}$)', color='blue', marker='o', ms=8)
        ...
        plt.legend()
        plt.savefig(os.path.join(args.results_path, f'RE_ER_diff_average_k(1).png'))
