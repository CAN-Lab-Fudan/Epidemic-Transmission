...
def main():
    ...
    with plt.style.context(['science', "ieee"]):
        figure = plt.figure()
        ax = plt.subplot()
        ax.plot(df_out.index.values, df_out.values, color='yellow', label='TGNN', ls='-.')
        ax.plot(df_mean.index.values, df_mean.values, color='black', label='Mean', ls='-')

        ax.axvline(x=threshold_QMF, ls=':', color='red',
        ...
        ax.axvline(x=lambda_c_pre, ls='--', color='blue',
                   label=r'$\lambda_c^\prime$=' + str(round(lambda_c_pre, 3)))
        plt.xlabel(r'$\lambda$')
        plt.ylabel(r'$n$')
        plt.text(0.95, 0.05, '(a)', horizontalalignment='center', verticalalignment='center', transform=plt.gca().transAxes)

        plt.legend()
        plt.savefig(os.path.join(args.results_path, group_name + '_train.png'))

        figure = plt.figure()
        plt.plot(df_var.index.values, df_var.values, color='black', ls='-', label=r'$I(\lambda)$')
        plt.axvline(x=lambda_c_pre, ls='--', color='blue',
                   label=r'$\lambda_c^\prime$=' + str(round(lambda_c_pre, 3)))
        
        plt.xlabel(r'$\lambda$')
        plt.ylabel(r'Output of $f_{det}$')
