...
def main():
    ...
    # edge_list = np.loadtxt(os.path.join(real_natworks_path, 'email-Eu-core.txt'), dtype=int)
    # G = nx.from_edgelist(edgelist=edge_list)
    G = nx.read_gml(os.path.join(real_natworks_path, 'power.gml'), label='id')
    ...
    with plt.style.context(['science', 'ieee']):
        figure = plt.figure()
        # plt.title(r'The results on Power network')
        ax = plt.subplot()
        # ax.plot(df_out.index.values, df_out.values, color='blue', label='output by NN', ls='-.')
        ax.plot(df_mean.index.values, df_mean.values, color='red', label=r'$n$', ls='-')
        # ax.axvline(x=threshold_QMF, ls='--', color='blue',
        #         label=r'$\lambda_c^{QMF}$=' + str(round(threshold_QMF, 5)))
        ax.axvline(x=threshold_HMF, ls='-.', color='green',
