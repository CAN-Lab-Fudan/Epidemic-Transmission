...
def main():
    ...
    edge_csv = np.loadtxt(os.path.join(real_natworks_path, 'soc-sign-bitcoinalpha.csv'), delimiter=',', dtype=int)
    edge_list = edge_csv[:, :2]
    ...
    with plt.style.context(['science', 'ieee']):
        figure = plt.figure()
        plt.title(r'The results on bitcoinalpha network')
        ax = plt.subplot()
